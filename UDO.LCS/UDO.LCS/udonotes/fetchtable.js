var FetchTable = function (options) {
    ...
    function openEntityForm(targetEntity, id) {
        var url = Xrm.Page.context.getClientUrl() + "/main.aspx?etn=" + targetEntity + "&pagetype=entityrecord&id=" + id;
        window.open(url);
    }
    ...
    function getFilterConditions() {
        ...
        for (var j = 0; j < _settings.columns.length; j++) {
            ...
            if (!UndefinedNullOrEmpty(column.filter.conditions)) {
                filterconditions = filterconditions.concat(column.filter.conditions);
            }
        }
    ...
    function refreshtable(clearcookie, data) {
        ...
        var footer = function (width, more, page, cookie) {
            ...
            var setRange = function (data) {
                ...
                var total = data.entities[0].total;
                ...
                if (isFiltered()) {
                    var setTotal = function (data) {
                        cell.append('&nbsp;&nbsp;', el('span', 'recordcount', '(Total Unfiltered Records: ' + data.entities[0].total + ')'));
                        $(".refreshedon").append(" - Total: " + data.entities[0].total + " - Filtered To: " + total);
                    }
        ...
        var contents = function (data, columns, tablewidth) {
            var tbody = el('tbody');
            if (data.entities != null) for (var i = 0; i < data.entities.length; i++) {
                            ...
                            switch (col.datatype) {
                                case 'datetime':
                                    if (Object.prototype.toString.call(v) === '[object Date]') {
                                        ...
                                        cell.text(days[v.getDay()] + ", " + month + "/" + v.getDate() + '/' + v.getFullYear() + ' ' + v.toLocaleTimeString());
                                    } else {
                                        cell.text('');
                                    }
                                    break;
