    ...
    XrmTable.Table = function (element, options) {
        ...
        function _updatePagingInfo() {
            if (_internal.query instanceof Sdk.Query.QueryExpression) {
                ...
                var fetchXmlString = fetchXml.toXmlString();
                _internal.query.setFetchXml(fetchXmlString);
                _log("FetchXML Query: " + fetchXmlString);
            }
        }
        ...
        function setViewRecordCount(data) 
        {
            ...
            if (data && data.getCount) {
                _internal.viewRecordCount = data.getCount();
            } else {
        ...
        function _updateOrderExpressions() {
            ...
            if (_internal.query instanceof Sdk.Query.QueryExpression) {
                ...
                for (i in order) {
                    if (order[i].extra) continue;
                    orderby.add(new Sdk.Query.OrderExpresison(order[i].key,
                        order[i].desc ? Sdk.Query.OrderType.Descending : Sdk.Query.OrderType.Ascending));
                }
                ...
                for (i in order) {
                    ...
                    if (attribute !== null) {
                        orderxml = '<order attribute="' + order[i].key + '"';
                        if (order[i].desc) {
        ...
        function _renderCards(data) {
            ...
            if (data instanceof Sdk.Entity) {
                waitfor.push(createCard(data.getId(), data).then(appendRow));
                ...
                for (i = 0; i < count; i++) {
                    waitfor.push(createCard(data.getEntity(i).getId(), data.getEntity(i)).then(appendRow));
                }
            } else if (data.push !== undefined) {
                for (i in data) {
            ...
            function createTableHeader(parent, cols, data) {
                ...
                function createColumnHeader(i, col) {
                    ...
                    if (order.length > 0) {
                        ...
                        //if (col.getKey() == order[0].col.getKey()) {
