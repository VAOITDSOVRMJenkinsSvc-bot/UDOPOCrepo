...
UTIL.trimChars = function(fieldName, pattern, message) {
    ...
    if (Xrm.Page.getAttribute(fieldName) && Xrm.Page.getAttribute(fieldName).getValue() != null) {
...
//Defaults to AlphaNum
        ...
        var origString = Xrm.Page.getAttribute(fieldName).getValue();
...
UTIL.registerCheckboxClick = function (attr) { // fireOnChange no longer needed as of RU12 and therefore this function can be removed
...
UTIL.isProperDate = function(fieldName) {
    var dtStr = Xrm.Page.getAttribute(fieldName).getValue();
    if ((dtStr == null) || (dtStr == '')) {
        return false;
    }
...
UTIL.ReplaceFieldTokens = function(qw) {
    ...
    while (pos != -1) {
        ...
        var attr = Xrm.Page.getAttribute(token);
        var attrVal = '';
...
UTIL.GetLookupId = function(lookupAttributeName) {
    var id = null;
    if (Xrm.Page.getAttribute(lookupAttributeName).getValue() &&
        Xrm.Page.getAttribute(lookupAttributeName).getValue().length > 0 &&
        Xrm.Page.getAttribute(lookupAttributeName).getValue()[0]) {
        id = Xrm.Page.getAttribute(lookupAttributeName).getValue()[0].id;
...
UTIL.createDynamicPicklist = function(varPicklist, varSubPicklist, varRelationArray) {
    var oPicklist = Xrm.Page.getAttribute(varPicklist);
    var oSubControl = Xrm.Page.getControl(varSubPicklist);
    var oSavedSubList = Xrm.Page.getAttribute(varSubPicklist).getOptions();
