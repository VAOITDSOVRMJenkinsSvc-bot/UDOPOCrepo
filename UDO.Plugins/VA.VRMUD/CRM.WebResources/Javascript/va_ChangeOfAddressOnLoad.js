...
function onFormLoad() {
    ...
    var webResourceUrl = parent.Xrm.Page.context.getServerUrl() + '/WebResources/va_';
    ...
    if (Xrm.Page.ui.getFormType() == CRM_FORM_TYPE_CREATE) {
        ...
        try {
            ...
            var ptcpntIdSearchCtx = new vrmContext();
            ...
            GeneralToolbar.AddButton("btnAddCopy", "Find Bank", "100%", va_routingnumberChange, webResourceUrl + 'money_add.png');
...
function validateAddressUsingWS(addressType, addressParams, responseFieldName, scoreFieldName) {
    var addressValidationCtx = new vrmContext();
    addressValidationCtx.user = GetUserSettingsForWebservice();
...
function RetrieveAwardAddresses(vetSearchCtx, opener) {
    ...
    if (parent_page != null && routingNumber != null && routingNumber == parent_page.getAttribute("va_crn").getValue()) {
...
function retrieveAppellantAddress() {
    var searchByValue = Xrm.Page.getAttribute("va_findappealsby").getValue(),
        ...
        appealsSearchContext = new vrmContext();
...
function ViewScript() {
    var scriptSource = Xrm.Page.context.getServerUrl().replace(Xrm.Page.context.getOrgUniqueName(), '') + 'isv/scripts_VIP/' + 'changeOfAddress.html';
