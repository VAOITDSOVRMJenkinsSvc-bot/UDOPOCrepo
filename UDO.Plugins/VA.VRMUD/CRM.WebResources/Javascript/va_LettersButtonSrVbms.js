...
var VbmsFileUpload = {
    ...
    CreateNote: function (controlId) {
        ...
        var ssn = Xrm.Page.getAttribute("va_filenumber").getValue();
        if (ssn != null) {
            if (confirm("Did you intend to upload the \"" + fileName + "\" letter for xxx-xx-" + ssn.substr(ssn.length - 4))) { // call ws to get the attachment file names from shared folder
                var names = '';
                var request = null;
                try {
                ...
                /* setInterval(function () {
                 var noteUpdate = new Object();
                 note.Subject = "VbmsFileUploadUpdate";
                 //VbmsFileUpload.CreateEntity(noteUpdate, "Annotation", "(guid'" + noteId.toUpperCase() + "')");
                 VbmsFileUpload.UpdateNotes(noteId);*/
                setInterval(function () {
