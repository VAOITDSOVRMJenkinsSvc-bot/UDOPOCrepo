...
namespace CRM.Plugins.ChatCoBrowseSessionLog
{
    ...
    {
        //public static bool ValidateChatSessionRecord(crme_chatcobrowsesessionlog chatCoBrowse, IPluginExecutionContext context)
        //{
        //    if (chatCoBrowse == null)
        //    {
        //        throw new InvalidPluginExecutionException("Chat session record is null");
        //    }
        //    //20 is a PreStage
        //    if (context.Stage == 20 && context.InputParameters.Contains("crme_ChatSessionId") == false) 
        //    {
        //        throw new InvalidPluginExecutionException("Chat session ID not specified");
        //    }
        //    //40 is a PostStage
        //    if (context.Stage == 40 && context.InputParameters.Contains("crme_ChatSessionId") == true)
        //    {
        //        if (chatCoBrowse.crme_ChatSessionId == null || chatCoBrowse.crme_ChatSessionId.Trim() == String.Empty)
        //        {
        //            throw new InvalidPluginExecutionException("Chat session ID not specified");
        //        }
        ...
        public static void getSensitivityLevel(PhoneCall phoneCall, string str)
        {   // "-" (a dash) means the participant is not sensitive
            // "Y" means the participant is sensitive but the user has access
            // "O" means the participant is sensitive, the user has access, but the user is trying to view their own record
            // "S" means the participant is sensitive, the user has access, but the user is trying to view the record of another user at their station
            // "P" means there is a parameter error
            // "E" means there was a general error
            //  A number 1-9 means the participant is sensitive, the user cannot view the record, and the user would need that level to access the record

            int? va_veteransensitivitylevel = null;

            try
            {
                va_veteransensitivitylevel = Convert.ToInt32(str);
            }
            catch (Exception)
            {
            }

            phoneCall.va_SensitivityLevelValue = va_veteransensitivitylevel;

        }

        public static void parseCallTypes(PhoneCall phoneCall, crme_chatcobrowsesessionlog chatCoBrowse)
        {
        ...
        public static OptionSetValue setCallType(string callType)
        {
            int iCallType = 0;
            switch (callType)
            {
        ...
        public static OptionSetValue setCallSubType(string callType, string callSubType)
        {
            int iCallSubType = 0;
            switch (callType)
            {
