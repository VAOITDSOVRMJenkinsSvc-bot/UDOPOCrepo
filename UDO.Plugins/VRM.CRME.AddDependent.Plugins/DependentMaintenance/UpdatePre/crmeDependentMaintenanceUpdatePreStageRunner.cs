...
namespace VRM.CRME.Plugin.DependentMaintenance
{
    ...
    public class crmeDependentMaintenanceUpdatePreStageRunner : MCSPlugins.PluginRunner
    {
        ...
        internal void Execute(IServiceProvider serviceProvider)
        {
            ...
            //    return;
            //}
            if (!GetPrimaryEntity().Attributes.Contains("crme_txnstatus"))
            {
                Logger.WriteDebugMessage("Status equal to null - must be save");
                return;
            }
        ...
        private bool ChecForFailureorSuccess()
        {
            ...
            var dependendentMaintenance = GetPrimaryEntity().ToEntity<crme_dependentmaintenance>();
        ...
        private void ValidateHasDependents()
        {
            ...
            var hasDependents = (from d in OrganizationServiceContext.CreateQuery<crme_dependent>()
                where (d.crme_DependentMaintenance.Id == id) &&
                      (d.crme_LegacyRecord == false  || (d.crme_LegacyRecord == true && d.crme_MaintenanceType.Value == 752280000) || (d.crme_LegacyRecord == true && d.crme_MaintenanceType.Value == 935950001))
                select d).ToList().Count > 0;

            var dm = GetPrimaryEntity().ToEntity<crme_dependentmaintenance>();
            var status = dm.crme_txnStatus;

            // Only perform validation here if DM transaction is being submitted
            if (status.Value == 935950001)
            {
                if (!hasDependents)
        ...
        private void ValidateMaritalInfo()
        {
            ...
            var openMarriages = (from d in OrganizationServiceContext.CreateQuery<crme_dependent>()
                                 where (d.crme_DependentMaintenance.Id == id) &&
                                 (d.udo_marriageenddate == null)  && (d.crme_DependentRelationship.Value == 935950001) &&
                                 (d.crme_AwardInd != "N")
                                 select d.Id).ToList();
        ...
        private void ValidateVeteran()
        {
            ...
            var status = veteran.crme_txnStatus;

            // Only perform field checks here if DM transaction is being submitted
            if (status.Value == 935950001)
            {
        ...
        private void CheckTransactionStatus()
        {
            ...
            {
                 if(GetTransactionStatus() == _Submitted) {
                    return;
                }
                else{
                    throw new InvalidPluginExecutionException(
