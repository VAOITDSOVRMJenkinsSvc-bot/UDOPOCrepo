...
{
    internal class CADDRetrieveMultiplePreRunner : PluginRunner
    {
        Guid _veteranId = new Guid();
        string _ssn = "";
        const string Query = "Query";
        public CADDRetrieveMultiplePreRunner(IServiceProvider serviceProvider)
            ...
            try
            {
                ...
                if (PluginExecutionContext.InputParameters["Query"] is QueryExpression)
                {
                    foreach (var item in criteria)
                    {
                        ...
                        if (item.AttributeName.Equals("va_caddssn"))
                        {
                        ...
                        if (item.AttributeName.Equals("udo_veteranid"))
                        {
                            var condValues = item.Values;
                            foreach (var item2 in condValues)
                            {
                    ...
                    if (didWeNeedData())
                    {
                        if (blnBefore)
                        {
                            ...
                            query.LinkEntities.Add(new LinkEntity
                            {
                                ...
                                LinkFromAttributeName = "va_accountownerid",
                                LinkFromEntityName = "va_bankaccount",
                        ...
                        else
                        {
                           ...
                           // Logger.WriteDebugMessage("on to linked entity");
                            ...
                            {
                                JoinOperator = Microsoft.Xrm.Sdk.Query.JoinOperator.Inner,
                                LinkFromAttributeName = "udo_veteranid",
                                LinkFromEntityName = "va_intenttofile",
                                LinkToAttributeName = "contactid",
                                LinkToEntityName = "contact",
                                LinkCriteria = new FilterExpression
                                {
