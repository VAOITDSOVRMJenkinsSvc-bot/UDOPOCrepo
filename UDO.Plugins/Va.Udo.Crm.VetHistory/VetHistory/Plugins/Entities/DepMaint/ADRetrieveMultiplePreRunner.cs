...
{
    internal class ADRetrieveMultiplePreRunner : PluginRunner
    {
        Guid _veteranId = new Guid();
        string _ssn = "";
        const string Query = "Query";
        public ADRetrieveMultiplePreRunner(IServiceProvider serviceProvider)
            ...
            try
            {
                ...
                if (PluginExecutionContext.InputParameters["Query"] is QueryExpression)
                {
                    foreach (var item in criteria)
                    {
                        ...
                        if (item.AttributeName.Equals("crme_relatedveteranid"))
                        {
                            var condValues = item.Values;
                            foreach (var item2 in condValues)
                            {
                    ...
                    if (didWeNeedData())
                    {
                        if (blnBefore)
                        {
                            ...
                            query.LinkEntities.Add(new LinkEntity
                            {
                                ...
                                LinkFromEntityName = "crme_dependentmaintenance",
