...
{
    internal class ITFRetrieveMultiplePreRunner : PluginRunner
    {
        Guid _veteranId = new Guid();
        string _ssn = "";
        const string Query = "Query";
        public ITFRetrieveMultiplePreRunner(IServiceProvider serviceProvider)
            ...
            try
            {
                ...
                if (PluginExecutionContext.InputParameters["Query"] is QueryExpression)
                {
                    foreach (var item in criteria)
                    {
                        ...
                        if (item.AttributeName.Equals("va_veteranssn"))
                        {
