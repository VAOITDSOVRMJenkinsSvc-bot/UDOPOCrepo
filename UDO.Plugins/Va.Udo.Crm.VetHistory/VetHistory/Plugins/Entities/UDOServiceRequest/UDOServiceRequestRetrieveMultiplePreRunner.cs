...
{
    internal class UDOServiceRequestRetrieveMultiplePreRunner : PluginRunner
    {
        Guid _veteranId = new Guid();
        string _ssn = "";
        const string Query = "Query";
        public UDOServiceRequestRetrieveMultiplePreRunner(IServiceProvider serviceProvider)
            ...
            try
            {
                ...
                if (PluginExecutionContext.InputParameters["Query"] is QueryExpression)
                {
                    foreach (var item in criteria)
                    {
                        ...
                        if (item.AttributeName.Equals("udo_ssn"))
                        {
                        ...
                        if (item.AttributeName.Equals("udo_relatedveteranid"))
                        {
                            var condValues = item.Values;
                            foreach (var item2 in condValues)
                            {
                ...
                if (blnContinue)
                {
                    if (didWeNeedData())
                    {
                        ...
                        query.LinkEntities.Add(new LinkEntity
                        {
                            ...
                            LinkFromAttributeName = "udo_relatedveteranid",
                            LinkFromEntityName = "udo_servicerequest",
