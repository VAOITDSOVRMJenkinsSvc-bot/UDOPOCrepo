...
{
    internal class PhoneCallRetrieveMultiplePreRunner : PluginRunner
    {
        Guid _veteranId = new Guid();
        string _ssn = "";
        const string Query = "Query";
        public PhoneCallRetrieveMultiplePreRunner(IServiceProvider serviceProvider)
            ...
            try
            {
                ...
                if (PluginExecutionContext.InputParameters["Query"] is QueryExpression)
                {
                    ...
                    if (McsSettings.getDebug)
                    {
                        ...
                        Logger.WriteDebugMessage("QueryExpression before I mess with it connects don't count- fetchxml = " + qResponse.FetchXml);
                    }
                ...
                if (PluginExecutionContext.InputParameters["Query"] is QueryExpression)
                {
                    foreach (var item in criteria)
                    {
                        ...
                        if (item.AttributeName.Equals("regardingobjectid"))
                        {
                            var condValues = item.Values;
                            foreach (var item2 in condValues)
                            {
