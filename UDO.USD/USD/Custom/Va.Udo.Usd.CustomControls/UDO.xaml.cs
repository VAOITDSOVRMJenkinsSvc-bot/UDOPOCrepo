...
namespace Va.Udo.Usd.CustomControls
{
    ...
    public partial class UDO : BaseHostedControlCommon
    {
        /// <summary>
        ///     Log writer
        /// </summary>
        ...
        #region Hardware Info Variables 

        string getProcessorId = string.Empty;
        string getHDDSerialNo = string.Empty;
        string getMACAddress = string.Empty;
        string getBoardMaker = string.Empty;
        string getBoardProductId = string.Empty;
        string getCdRomDrive = string.Empty;
        string getBIOSmaker = string.Empty;
        string getBIOSserNo = string.Empty;
        string getBIOScaption = string.Empty;
        string getAccountName = string.Empty;
        string getPhysicalMemory = string.Empty;
        string getNoRamSlots = string.Empty;
        string getCPUManufacturer = string.Empty;
        int getCPUCurrentClockSpeed = 0;
        string getDefaultIPGateway = string.Empty;
        double? getCpuSpeedInGHz = 0;
        string getCurrentLanguage = string.Empty;
        string getOSInformation = string.Empty;
        string getProcessorInformation = string.Empty;
        string getComputerName = string.Empty;
        string getIPAddress = string.Empty;

        #endregion

        string USDVersion = string.Empty;
        string OSType = string.Empty;
        string screenWidth = string.Empty;
        string screenHeight = string.Empty;
        ...
        public UDO(Guid id, string applicationName, string initXml) :
        ...
        protected override void DesktopReady()
        {
            ...
            RegisterAction("StartTrackEvent", ManagementInfo);
            RegisterAction("EndTrackEvent", ManagementInfo);
            RegisterAction("TelemetryEvent", ManagementInfo);
            RegisterAction("TelemetryTrace", ManagementInfo);

            #region Hardware Info 
            
            getPhysicalMemory = HardwareInfo.GetPhysicalMemory();
            UpdateSessionContext(localSessionManager.ActiveSession, "HardwareInfo", "PhysicalMemory", getPhysicalMemory);

            getOSInformation = HardwareInfo.GetOSInformation();
            UpdateSessionContext(localSessionManager.ActiveSession, "HardwareInfo", "OSInformation", getOSInformation);

            getProcessorInformation = HardwareInfo.GetProcessorInformation();
            UpdateSessionContext(localSessionManager.ActiveSession, "HardwareInfo", "ProcessorInformation", getProcessorInformation);

            getComputerName = HardwareInfo.GetComputerName();
            UpdateSessionContext(localSessionManager.ActiveSession, "HardwareInfo", "ComputerName", getComputerName);

            USDVersion = GetFileVersion(Assembly.GetEntryAssembly()).ToString();
            OSType = System.Environment.Is64BitProcess ? "64bit" : "32bit";
            ...
            foreach (var ip in ipEntry.AddressList)
            {
                ...
                {
                    UpdateSessionContext(localSessionManager.ActiveSession, "HardwareInfo", "IPAddress", ip.ToString());
                    getIPAddress = ip.ToString();
                    break;
                }
            }

            screenWidth = System.Windows.SystemParameters.PrimaryScreenWidth.ToString();
            screenHeight = System.Windows.SystemParameters.PrimaryScreenHeight.ToString();
        ...
        {
            if (string.Compare(args.Action, "CheckIESettings", StringComparison.OrdinalIgnoreCase) == 0)
            {
                CheckIESettings(args);
            }
            else if (string.Compare(args.Action, "CheckUSDVersion", StringComparison.OrdinalIgnoreCase) == 0)
            {
                CheckUSDVersion(args);
            }
            else if (string.Compare(args.Action, "SubmitITF", StringComparison.OrdinalIgnoreCase) == 0)
            {
                submitITF(args);
            }
            else if (string.Compare(args.Action, "TelemetryEventStart", StringComparison.OrdinalIgnoreCase) == 0)
            {
                TelemetryEventStart(args);
            }
            else if (string.Compare(args.Action, "TelemetryEventEnd", StringComparison.OrdinalIgnoreCase) == 0)
            {
                TelemetryEventEnd(args);
            }
            else if (string.Compare(args.Action, "TelemetryEvent", StringComparison.OrdinalIgnoreCase) == 0)
            {
                TelemetryEvent(args);
            }
            else if (string.Compare(args.Action, "TelemetryTrace", StringComparison.OrdinalIgnoreCase) == 0)
            {
                TelemetryTrace(args);
            }
        ...
        private void CheckIESettings(RequestActionEventArgs args)
        {
            ...
            displayMessage = Utility.GetAndRemoveParameter(lines, "DisplayMessage");
            ...
            try
            {
                results.Add("DisplayMessage", displayMessage.ToString());
        ...
        private void submitITF(RequestActionEventArgs args)
        {
            var parms = Utility.SplitLines(args.Data, CurrentContext, localSession);
            var customActionName = Utility.GetAndRemoveParameter(parms, "CustomActionName");
            var datanodename = Utility.GetAndRemoveParameter(parms, "DataNodeName");
            var parent = Utility.GetAndRemoveParameter(parms, "ParentEntityReference").ToEntityReference();

            try
            {

                base.UpdateContext(datanodename, "Timeout", "");
                base.UpdateContext(datanodename, "Exception", "");
                base.UpdateContext(datanodename, "DataIssue", "");
                base.UpdateContext(datanodename, "ResponseMessage", "");
                base.UpdateContext(datanodename, "result", "");


                var req = new OrganizationRequest(customActionName);
                req["Target"] = parent;
                req["ParentEntityReference"] = parent;

                var response = base.Execute(req);

                if (response.Results.Contains("Timeout"))
                {
                    var timeOut = (bool)response["Timeout"];
                    base.UpdateContext(datanodename, "Timeout", timeOut.ToString());
                }

                if (response.Results.Contains("Exception"))
                {
                    var exception = (bool)response["Exception"];
                    base.UpdateContext(datanodename, "Exception", exception.ToString());
                }

                if (response.Results.Contains("DataIssue"))
                {
                    var dataIssue = (bool)response["DataIssue"];
                    base.UpdateContext(datanodename, "DataIssue", dataIssue.ToString());
                }

                if (response.Results.Contains("ResponseMessage") && response.Results["ResponseMessage"] != null)
                {
                    var repoonseMessage = response.Results["ResponseMessage"].ToString();
                    base.UpdateContext(datanodename, "ResponseMessage", repoonseMessage.ToString());
                }

                if (response.Results.Contains("result") && response.Results["result"] != null)
                {
                    var result = (EntityReference)response.Results["result"];
                    base.UpdateContext(datanodename, "result", result.ToString());
                }

                if (response.Results.Contains("createDt") && response.Results["createDt"] != null)
                {
                    var createDt = response.Results["createDt"];
                    base.UpdateContext(datanodename, "createDt", createDt.ToString());
                }

                if (response.Results.Contains("jrnLctnId") && response.Results["jrnLctnId"] != null)
                {
                    var jrnLctnId = response.Results["jrnLctnId"];
                    base.UpdateContext(datanodename, "jrnLctnId", jrnLctnId.ToString());
                }

                if (response.Results.Contains("jrnUserId") && response.Results["jrnUserId"] != null)
                {
                    var jrnUserId = response.Results["jrnUserId"];
                    base.UpdateContext(datanodename, "jrnUserId", jrnUserId.ToString());
                }

                if (response.Results.Contains("submtrApplcnTypeCd") && response.Results["submtrApplcnTypeCd"] != null)
                {
                    var jrnUserId = response.Results["submtrApplcnTypeCd"];
                    base.UpdateContext(datanodename, "submtrApplcnTypeCd", jrnUserId.ToString());
                }
            }
            catch (Exception ex)
            {
                ...
                base.UpdateContext(datanodename, "ExceptionMessage", "Unexpected Error Occurred");
                base.UpdateContext(datanodename, "ExceptionReport", exp);
            }
        }

        private void TelemetryEventStart(RequestActionEventArgs args)
        {
            Session currentSession;
            var lines = Utility.SplitLines(args.Data, CurrentContext, localSession);
            var dataNodeName = Utility.GetAndRemoveParameter(lines, "datanodename");
            var startEvent = Utility.GetAndRemoveParameter(lines, "StartEvent").Trim();
            var strIsGlobal = Utility.GetAndRemoveParameter(lines, "IsGlobal").Trim();
            bool isGlobal = false;

            #region Check If Global vs Local Session

            if (!string.IsNullOrEmpty(strIsGlobal))
            {
                if (strIsGlobal == "True")
                {
                    isGlobal = true;
            ...
            if (isGlobal)
            {
                currentSession = localSessionManager.GlobalSession;
            }
            else
            {
                currentSession = localSessionManager.ActiveSession;
            }

            #endregion

            #region Store extra variables in Data Node

            foreach (var line in lines)
            {
                UpdateSessionContext(currentSession, dataNodeName, line.Key, line.Value);
            }

            #endregion

            UpdateSessionContext(currentSession, dataNodeName, "StartEvent", startEvent);
            UpdateSessionContext(currentSession, dataNodeName, "StartEvent_StartTime", DateTime.UtcNow.ToString("O"));

            //TelemetryTimer = new Stopwatch();
            //TelemetryTimer.Start();

        }

        private void TelemetryEventEnd(RequestActionEventArgs args)
        {
            Session currentSession;
            var lines = Utility.SplitLines(args.Data, CurrentContext, localSession);
            var dataNodeName = Utility.GetAndRemoveParameter(lines, "datanodename");
            var endEvent = Utility.GetAndRemoveParameter(lines, "EndEvent").Trim();
            var strIsGlobal = Utility.GetAndRemoveParameter(lines, "IsGlobal").Trim();
            bool isGlobal = false;
            ...
            UpdateSessionContext(currentSession, dataNodeName, "EndEvent", endEvent);
            UpdateSessionContext(currentSession, dataNodeName, "EndEvent_EndTime", DateTime.UtcNow.ToString("O"));

            var start = GetDataParameter(currentSession, dataNodeName, "StartEvent_StartTime");
            var end = GetDataParameter(currentSession, dataNodeName, "EndEvent_EndTime");
            ...
            if (!string.IsNullOrEmpty(start) && !string.IsNullOrEmpty(end))
            {
                ...
                if (DateTime.TryParse(start, out startTime))
                {
                    if (DateTime.TryParse(end, out endTime))
                    {
                        ...
                        UpdateSessionContext(currentSession, dataNodeName, "ElapsedTime", duration.TotalSeconds.ToString());
                    }
                }
            }

            //TelemetryTimer.Stop();
            //UpdateSessionContext(currentSession, eventName, "TimerDuration", TelemetryTimer.ElapsedMilliseconds.ToString());
        }

        private void TelemetryEvent(RequestActionEventArgs args)
        {
            var configPath = GetSpecialFolder("GetUSDCustomizationPath");
            TelemetryConfiguration configuration = TelemetryConfiguration.CreateFromConfiguration(File.ReadAllText(Path.Combine(configPath, "ApplicationInsights.config")));
            configuration.TelemetryInitializers.Add(new HttpDependenciesParsingTelemetryInitializer());

            var lines = Utility.SplitLines(args.Data, CurrentContext, localSession);

            EventTelemetry data = new EventTelemetry("USD Evnt Unknown");
            data.Timestamp = DateTime.UtcNow;
            data.Context.User.Id = _client.CrmInterface.GetMyCrmUserId().ToString();

            data.Context.Device.Id = getComputerName;
            data.Context.Device.Model = getProcessorInformation;
            data.Context.Device.OperatingSystem = getOSInformation;
            
            data.Properties.Add("Ram", getPhysicalMemory);
            data.Properties.Add("OSType", OSType);
            data.Properties.Add("USDVersion", USDVersion);
            data.Properties.Add("IPAddress", getIPAddress);
            data.Properties.Add("IEVersion", GetIEVersion());
            data.Properties.Add("ScreenResolution", screenWidth + " X " + screenHeight);

            foreach (var line in lines)
            {
                if (line.Key.Equals("EventName"))
                {
                    data.Name = line.Value;
                }
                else if (line.Key.Equals("InstrumentationKey"))
                {
                    configuration.InstrumentationKey = line.Value;
                }
                else if (line.Key.Equals("SessionId"))
                {
                    data.Context.Session.Id = line.Value;
                }
                else if (line.Key.Equals("ElapsedTime"))
                {
                    double duration;
                    if (double.TryParse(line.Value, out duration))
                    {
                        data.Metrics.Add(line.Key, duration);
                    }
                }
                else if (line.Key.Equals("OperationName"))
                {
                    data.Context.Operation.Name = line.Value;
                }
                else
                {
                    data.Properties.Add(line.Key, line.Value);
                }
            }
        ...
        private void TelemetryTrace(RequestActionEventArgs args)
        {
            var configPath = GetSpecialFolder("GetUSDCustomizationPath");
            TelemetryConfiguration configuration = TelemetryConfiguration.CreateFromConfiguration(File.ReadAllText(Path.Combine(configPath, "ApplicationInsights.config")));
            configuration.TelemetryInitializers.Add(new HttpDependenciesParsingTelemetryInitializer());
            ...
            TraceTelemetry data = new TraceTelemetry("USD Trace Unknown");
            data.Timestamp = DateTime.UtcNow;
            data.Context.User.Id = _client.CrmInterface.GetMyCrmUserId().ToString();

            data.Context.Session.Id = localSession.SessionId.ToString();
            data.Context.Device.Id = getComputerName;
            data.Context.Device.Model = getProcessorInformation;
            data.Context.Device.OperatingSystem = getOSInformation;
            ...
            foreach (var line in lines)
            {
                if (line.Key.Equals("Message"))
                {
                    data.Message = line.Value;
                }
                else if (line.Key.Equals("InstrumentationKey"))
                {
                    configuration.InstrumentationKey = line.Value;
                    data.Context.InstrumentationKey = line.Value;
                }
                else
                {
                    data.Properties.Add(line.Key, line.Value);
            ...
            using (InitializeDependencyTracking(configuration))
            {
                telemetryClient.TrackTrace(data);
            }
