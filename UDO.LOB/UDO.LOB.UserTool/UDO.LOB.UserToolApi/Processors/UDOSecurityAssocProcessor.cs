...
namespace UDO.LOB.UserTool.Processors
{
    ...
    public class UDOSecurityAssocProcessor
    {
        ...
        public IMessageBase Execute(UDOSecurityAssocRequest request)
        {
            ...
            var response = new UDOSecurityAssocResponse { MessageId = request.MessageId };
            if (request == null)
            {
            ...
            if (request.DiagnosticsContext == null && request != null)
            {
                request.DiagnosticsContext = new DiagnosticsContext()
                {
                    ...
                    MessageTrigger = "UDOSecurityAssocProcessor",
            ...
            LogHelper.LogInfo($">> {this.GetType().FullName}.Execute: Request:  {JsonHelper.Serialize<UDOSecurityAssocRequest>(request)}");
        ...
        private void DoWork(CrmServiceClient orgService, UDOSecurityAssocRequest request)
        {
            try
            {
                ...
                LogHelper.LogDebug(request.MessageId, request.OrganizationName, request.UserId, method, "Executing Multiple:  \r\n\r\n" + LogBuffer, _debug);
            ...
            catch (Exception ex)
            {
                ...
                LogHelper.LogDebug(request.MessageId, request.OrganizationName, request.UserId, method, LogBuffer + " Processor, Progress:" + progressString, _debug);
            }
            finally
            {
                if (orgService != null)
        ...
        internal void AddUsers(IOrganizationService orgService, List<SecurityAssociation> associations, EntityReference securityRecord, IEnumerable<EntityReference> users)
        {
        ...
        internal void AddSecurityAccess(IOrganizationService orgService, List<SecurityAssociation> associations, EntityReference user,
        ...
        internal void AddQueueMembership(IOrganizationService orgService, IEnumerable<EntityReference> users,
        ...
        private IEnumerable<EntityReference> GetSecurityGroupQueues(IEnumerable<EntityReference> securityGroups,
            IOrganizationService orgService, UDOSecurityAssocRequest request)
        {
            var _debug = request.Debug;
            try
            {
        ...
        private EntityCollection Fetch(IOrganizationService orgService, bool debug, string fetch, string description = "")
        {
            if (debug)
            {
                Log("Fetching {0}... \r\n", description);
