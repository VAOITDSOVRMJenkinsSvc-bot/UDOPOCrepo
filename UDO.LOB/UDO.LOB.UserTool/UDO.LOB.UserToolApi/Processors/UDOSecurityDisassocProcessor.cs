...
namespace UDO.LOB.UserTool.Processors
{
    ...
    public class UDOSecurityDisassocProcessor
    {
        ...
        public IMessageBase Execute(UDOSecurityDisassocRequest request)
        {
            ...
            var response = new UDOSecurityDisassocResponse { MessageId = request.MessageId };
            ...
            LogHelper.LogInfo($">> {this.GetType().FullName}.Execute: Request:  {JsonHelper.Serialize<UDOSecurityDisassocRequest>(request)}");
            ...
            {
                LogHelper.LogError(request.MessageId, request.OrganizationName, request.UserId, string.Format("{0} Processor, Connection Error", method), connectException);
        ...
        private void DoWork(CrmServiceClient orgService, UDOSecurityDisassocRequest request)
        {
            try
            {
        ...
        internal IEnumerable<EntityReference> GetSecurityGroupUsers(IOrganizationService orgService, Guid securityGroupId)
        {
        ...
        internal void RemoveUsers(IOrganizationService orgService, List<SecurityAssociation> associations, EntityReference securityRecord, IEnumerable<EntityReference> users)
        {
        ...
        internal IEnumerable<Guid> GetAssignedRoles(IOrganizationService orgService, EntityReference user)
        {
        ...
        internal void RemoveSecurityAccess(IOrganizationService orgService, List<SecurityAssociation> associations, EntityReference user,
        ...
        internal void RemoveQueueMembership(IOrganizationService orgService, IEnumerable<EntityReference> users,
            IEnumerable<EntityReference> queueRecords, UDOSecurityDisassocRequest request)
        {
            ...
            {
                LogHelper.LogError(request.MessageId, request.OrganizationName, request.UserId, method, ex);
                LogHelper.LogDebug(request.MessageId, request.OrganizationName, request.UserId, method, $" Progress: { progressString} : LogBuffer: { LogBuffer}", _debug);
            }
        }

        private IEnumerable<EntityReference> GetSecurityGroupQueues(IEnumerable<EntityReference> securityGroups, IOrganizationService orgService, UDOSecurityDisassocRequest request)
        {
            var _debug = request.Debug;
            try
            {
        ...
        private EntityCollection Fetch(IOrganizationService orgService, bool _debug, string fetch, string description = "")
        {
            if (_debug)
            {
                Log("Fetching {0}... \r\n", description);
