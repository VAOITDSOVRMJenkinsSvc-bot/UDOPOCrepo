...
namespace UDO.LOB.ServiceRequest.Processors
{
    class UDOInitiateSRProcessor
    {
        ...
        public IMessageBase Execute(UDOInitiateSRRequest request)
        {
            ...
            LogHelper.LogInfo($">> {this.GetType().FullName}.Execute: Request:  {JsonHelper.Serialize<UDOInitiateSRRequest>(request)}");
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        //Use veteran contact record for the phone numbers instead of person record
                        var src_veteran_cols = new string[] { "udo_phonenumber1", "udo_phonenumber2" };

                        var dst_veteran_cols = new string[] { "udo_dayphone", "udo_eveningphone" };

                        var veteran = OrgServiceProxy.Retrieve("contact", sr_VeteranId.Value, new ColumnSet(src_veteran_cols));

                        MapFields(veteran, src_veteran_cols, serviceRequest, dst_veteran_cols);
                    }
                ...
                try
                {
                    if (VetSnapshotId.HasValue)
                    {
                        ...
                        var snap_dst_cols = new List<string>(new string[] {
                            ...
                            "udo_filenumber",
                            "udo_poadata",
                            "","udo_dateofdeath","" // udo_soj, udo_dateofdeath, udo_branchofservice
                        });
                        ...
                        if (!string.IsNullOrEmpty(snapShot.GetAttributeValue<string>("udo_dateofdeath")))
                        {
                            ...
                            if (DateTime.TryParse(snapShot.GetAttributeValue<string>("udo_dateofdeath"), out newDateTime))
                            {
                                if (newDateTime != System.DateTime.MinValue)
                                {
                                    ...
                                    {
                                        DateTime crmDateTime = Tools.ToCRMDateTime(newDateTime);
                                        UpdateProgress("Converted Date of Death - " + crmDateTime.ToShortDateString());
                                        serviceRequest["udo_dateofdeath"] = crmDateTime;
                                    }
                                }
                            }
                        }
                ...
                if (!serviceRequest.Contains("udo_emailofveteran") || String.IsNullOrEmpty(serviceRequest["udo_emailofveteran"].ToString()))
                {
                    ...
                    //var findCorporateRecordByFileNumberResponse = WebApiUtility.SendReceive<VEIScrpFNfindCorporateRecordByFileNumberResponse>(veisBaseUri, findCorporateRecordByFileNumberRequest.MessageId, findCorporateRecordByFileNumberRequest, logSettings);
                    ...
                    {
                        var corpRecord = findCorporateRecordByFileNumberResponse.VEIScrpFNreturnInfo;
                ...
                else
                {
                    ...
                    if (serviceRequest.Attributes.Contains("udo_vetfirstname") && serviceRequest.Attributes.Contains("udo_vetlastname"))
                    {
        ...
        private void UpdateAddress(UDOInitiateSRRequest request, Entity person)
        {
            ...
            // REM: Invoke VEIS Endpoint
            //var findAllPtcpntAddrsByPtcpntIdResponse = WebApiUtility.SendReceive<VEISvetPctfindVeteranByPtcpntIdResponse>(veisBaseUri, findAllPtcpntAddrsByPtcpntIdRequest.MessageId, findAllPtcpntAddrsByPtcpntIdRequest, logSettings);
            //var findAllPtcpntAddrsByPtcpntIdResponse = WebApiUtility.SendReceive<VEISvetPctfindVeteranByPtcpntIdResponse>(findAllPtcpntAddrsByPtcpntIdRequest, WebApiType.VEIS);
            ...
            if (request.LogSoap || findAllPtcpntAddrsByPtcpntIdResponse.ExceptionOccurred)
            {
                if (findAllPtcpntAddrsByPtcpntIdResponse.SerializedSOAPRequest != null || findAllPtcpntAddrsByPtcpntIdResponse.SerializedSOAPResponse != null)
                {
                    ...
                    LogHelper.LogDebug(request.MessageId, request.OrganizationName, request.UserId, MethodInfo.GetThisMethod().Method, $"VEISeFolderinitializeUploadRequest Request/Response {requestResponse}", true);
                }
            }
        ...
        private static EntityReference getSOJId(IOrganizationService OrgServiceProxy, string stationCode)
        {
        ...
        /// </summary>
        /// <param name="serviceRequest"></param>
        private static EntityReference GetDefaultCurrency(IOrganizationService orgService, UDOInitiateSRRequest request)
        {
