    ...
    class UDOvalidateAddressProcessor
    {
        ...
        public IMessageBase Execute(UDOvalidateAddressRequest request)
        {
            ...
            try
            {
                ...
                var validateAddressRequest = new VEISvaladdvalidateAddressRequest
                {
                    ...
                    OrganizationName = request.OrganizationName,
                    // Replaced: VIMT.AddressWebService.Messages.HeaderInfo
                ...
                // Replaced: var validateAddressResponse = validateAddressRequest.SendReceive<VIMTvaladdvalidateAddressResponse>(MessageProcessType.Local);
                var validateAddressResponse = WebApiUtility.SendReceive<VEISvaladdvalidateAddressResponse>(validateAddressRequest, WebApiType.VEIS);

                if (request.LogSoap || validateAddressResponse.ExceptionOccurred)
                {
                    if (validateAddressResponse.SerializedSOAPRequest != null || validateAddressResponse.SerializedSOAPResponse != null)
                    {
                        var requestResponse = validateAddressResponse.SerializedSOAPRequest + validateAddressResponse.SerializedSOAPResponse;
                ...
                response.ExceptionOccurred = validateAddressResponse.ExceptionOccurred;

                var requestCollection = new OrganizationRequestCollection();

                if (validateAddressResponse != null)
                {
                    if (validateAddressResponse.VEISvaladdreturnInfo != null)
                    {
                        ...
                        foreach (var validatedAddress in validateAddressResponse.VEISvaladdreturnInfo)
                        {
            ...
            catch (Exception ExecutionException)
            {
                LogHelper.LogError(request.OrganizationName, request.UserId, request.RelatedParentId, 
                    ...
                    "UDOvalidateAddress Processor, Progess:" + progressString, ExecutionException);
