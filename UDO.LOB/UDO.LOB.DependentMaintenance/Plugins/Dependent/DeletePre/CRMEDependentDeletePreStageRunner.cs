    ...
    public class CRMEDependentDeletePreStageRunner : MCSPlugins.PluginRunner
    {
        ...
        internal void Execute()
        {
            try
            {
                ...
                TracingService.Trace("See if status is Draft");

                //Get CrmeDependent Enttity
                CheckIfOkToDeleteBasedonStatus();

                TracingService.Trace("Make sure this isn't a legacy dependent");

                //Allow Deletion of Non-WebService Dependents
                CheckIfOkToDelete();

                TracingService.Trace("Delete any marital history records");
