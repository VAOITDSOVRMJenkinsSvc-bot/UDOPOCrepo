...
namespace VRM.Integration.UDO.Contact.Processors
{
    class UDOcreateAddressRecordsProcessor
    {
        ...
        {
            //var request = message as createAddressRecordsRequest;
            ...
            try
            {
                ...
                if (request.ptcpntId > 0)
                {
                    ...
                    if (findAllPtcpntAddrsByPtcpntIdResponse.VIMTfallpidaddpidreturnInfo != null)
                    {
                        ...
                        MapFindAddressResponse(findAllPtcpntAddrsByPtcpntIdResponse.VIMTfallpidaddpidreturnInfo, request);
                    }
                }
                else
                {
                    if (!string.IsNullOrEmpty(request.fileNumber))
                    {
                        ...
                        response.ExceptionOccured = findAllPtcpntAddrsByFNResponse.ExceptionOccured;

                        if (findAllPtcpntAddrsByFNResponse.VIMTfallpidaddFNreturnInfo != null)
                        {
                            progressString = "Starting on mapping";
                            MapFindAddressResponse(findAllPtcpntAddrsByFNResponse.VIMTfallpidaddFNreturnInfo, request);
                        }
                    }
            ...
            {
                LogHelper.LogError(request.OrganizationName, request.UserId, request.RelatedParentId, request.RelatedParentEntityName, request.RelatedParentFieldName, "UDOcreateAddressRecordsProcessor Processor, Progess:" + progressString, ExecutionException);
        ...
        private void MapFindAddressResponse(VIMTfallpidaddFNreturnMultipleResponse[] ptcpntAddrsDTO, UDOcreateAddressRecordsRequest request)
        {
            foreach (var ptcpntAddrsDTOItem in ptcpntAddrsDTO)
            {
                ...
                //if (!string.IsNullOrEmpty(ptcpntAddrsDTOItem.mcs_countyNm))
                //{
                //    thisNewEntity["udo_country"] = ptcpntAddrsDTOItem.mcs_countyNm;
                //    if (thisisMailing)
                //    {
        ...
        private void MapFindAddressResponse(VIMTfallpidaddpidreturnMultipleResponse[] ptcpntAddrsDTO, UDOcreateAddressRecordsRequest request)
        {
            foreach (var ptcpntAddrsDTOItem in ptcpntAddrsDTO)
            {
                var thisisMailing = false;
        ...
        private VIMTfallpidaddpidfindAllPtcpntAddrsByPtcpntIdResponse FindAddressByPtcpntId(UDOcreateAddressRecordsRequest request)
        {
            ...
            return findAllPtcpntAddrsByPtcpntIdRequest.SendReceive<VIMTfallpidaddpidfindAllPtcpntAddrsByPtcpntIdResponse>(MessageProcessType.Local);

        }

        private VIMTfallpidaddFNfindAllPtcpntAddrsByFileNumberResponse FindAddressByFileNumber(UDOcreateAddressRecordsRequest request)
        {
            var findAllPtcpntAddrsByPtcpntIdRequest = new VIMTfallpidaddFNfindAllPtcpntAddrsByFileNumberRequest();
            ...
            // TODO(TN): Comment to remediate
            return findAllPtcpntAddrsByPtcpntIdRequest.SendReceive<VIMTfallpidaddFNfindAllPtcpntAddrsByFileNumberResponse>(MessageProcessType.Local);
        }
