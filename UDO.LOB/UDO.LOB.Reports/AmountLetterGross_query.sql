SELECT serreq.udo_reqnumber, 
	CASE WHEN LEN(serreq.udo_filenumber) = 9 AND serreq.udo_dateofdeath IS NOT NULL 
		THEN 'XSS' 
		ELSE '' 
		END + CASE WHEN LEN(serreq.udo_filenumber) < 9 AND serreq.udo_dateofdeath IS NOT NULL 
					THEN 'XC' 
					ELSE '' 
		END + CASE WHEN LEN(serreq.udo_filenumber) < 9 AND serreq.udo_dateofdeath IS NULL 
                    THEN 'C' 
					ELSE '' 
		END + CASE WHEN LEN(serreq.udo_filenumber) = 9 AND serreq.udo_dateofdeath IS NULL 
					THEN 'CSS' 
	...
	ISNULL(UPPER(SUBSTRING(serreq.udo_SRFirstName, 1, 1)) 
		+ SUBSTRING(LOWER(serreq.udo_SRFirstName), 2, LEN(serreq.udo_SRFirstName) - 1), '') + ' ' + ISNULL(UPPER(SUBSTRING(serreq.udo_SRLastName, 1, 1)) 
		+ SUBSTRING(LOWER(serreq.udo_SRLastName), 2, LEN(serreq.udo_SRLastName) - 1), '') 
	AS c_fullname, 
...
FROM udo_lettergeneration AS serreq WITH (nolock) 
	...
	-- BELOW used for finding optionset values, we will not use this
	LEFT OUTER JOIN StringMap AS udo_evaluationconsideredpermanentPLTable 
		ON udo_evaluationconsideredpermanentPLTable.AttributeName = 'udo_evaluationconsideredpermanent' 
			AND udo_evaluationconsideredpermanentPLTable.ObjectTypeCode IN
                             (SELECT ObjectTypeCode FROM EntityView WHERE (Name = 'udo_lettergeneration')) 
							 AND udo_evaluationconsideredpermanentPLTable.AttributeValue = serreq.udo_EvaluationConsideredPermanent 
	LEFT OUTER JOIN StringMap AS udo_contactprefixPLTable ON udo_contactprefixPLTable.AttributeName = 'udo_contactprefix' AND udo_contactprefixPLTable.ObjectTypeCode IN
