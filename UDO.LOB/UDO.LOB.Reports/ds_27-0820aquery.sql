SELECT
	...
	--CASE WHEN pc.va_calleridentityverified = 1 --Yes
	--		OR pc.va_filessnverified = 1 --Yes 
	...
	serreq.udo_readscriptname AS ReadScript,
	
	vet.firstname AS firstname,
	vet.lastname AS lastname,
	vet.middlename AS middlename,
	serreq.udo_srfirstname + ' ' + serreq.udo_srlastname AS FNODReportingForName,
	serreq.udo_requesttype + '-' + serreq.udo_requestsubtype AS RequestTypeSubType,
	
	ISNULL(serreq.udo_typeofcontact, '752280001') AS TypeOfContact,
	
	-- [udo_relationtoveteranPLTable].Value AS VetRelation,
	serreq.udo_relationtoveteranname AS VetRelation,
	
	Convert(VarChar, serreq.udo_dateofdeath, 101) AS DOD,
	
	-- [udo_enroutetovaPLTable].Value AS EnRoute,
	serreq.udo_enroutetovaname AS EnRoute,
	...
	serreq.udo_benefitsstopfirstofmonthname AS BenefitsStopped,

	-- [udo_possibleburialinnationalcemeteryPLTable].Value AS BurialBenefits,
	serreq.udo_possibleburialinnationalcemeteryname AS BurialBenefits,

	-- [udo_willroutereportofdeathPLTable].Value AS WillRouteReport,
	serreq.udo_willroutereportofdeathname AS WillRouteReport,
...
FROM 
	...
	-- Below is used to pull optionset names, we will use name field
		left outer join StringMap [udo_ReadScriptPLTable] on 
        ([udo_ReadScriptPLTable].AttributeName = 'udo_readscript'
			and [udo_ReadScriptPLTable].ObjectTypeCode in (SELECT ObjectTypeCode FROM EntityView where Name='udo_servicerequest') -- ObjectTypeCode = 10197
