SELECT serreq.udo_reqnumber
	...
	,ISNULL(UPPER(SUBSTRING(serreq.udo_FirstName, 1, 1)) + SUBSTRING(LOWER(serreq.udo_FirstName), 2, LEN(serreq.udo_FirstName) - 1), '') + ' ' + ISNULL(UPPER(SUBSTRING(serreq.udo_LastName, 1, 1)) + SUBSTRING(LOWER(serreq.udo_LastName), 2, LEN(serreq.udo_LastName) - 1), '') AS c_fullname
