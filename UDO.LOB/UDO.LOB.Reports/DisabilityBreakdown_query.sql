SELECT      
	...
	ISNULL(UPPER(SUBSTRING(vet.FirstName, 1, 1)) + SUBSTRING(LOWER(vet.FirstName), 2, LEN(vet.FirstName) - 1), '') 
            + ' ' + ISNULL(UPPER(SUBSTRING(vet.MiddleName, 1, 1)), '') + (CASE WHEN vet.middlename IS NULL THEN '' ELSE ' ' END) + ISNULL(UPPER(SUBSTRING(vet.LastName, 1, 1)) 
            + SUBSTRING(LOWER(vet.LastName), 2, LEN(vet.LastName) - 1), '') 
	AS c_fullname, 
	...
	CASE WHEN serreq.udo_LetterAddressing = 953850001 THEN pc.udo_returnmailingaddress ELSE ic.udo_returnmailingaddress 
	...
	serreq.udo_DiagnosticCodes AS DiagnosticCodes, 
...
FROM udo_lettergeneration AS serreq WITH (nolock) INNER JOIN
	...
	-- UDODEVTN: Below here is effort to look up option set values. We will not use this.
