...
function OnLoad(execCon) {
    ...
    exCon = execCon;
    formContext = exCon.getFormatContext;
    var dateofDeath = Xrm.Page.getAttribute("udo_dateofdeath");

    // Kirk
    //var isCreateForm = (Xrm.Page.ui.getFormType() == 1);
    //var isUpdateForm = (Xrm.Page.ui.getFormType() == 2);
    // if (isCreateForm === true) {
    if (dateofDeath === null || dateofDeath.getValue() === null) {
        // if (isCreateForm === true) {

        // Service Request Content Info
        getA("va_nameofreportingindividual").setValue(null);
        getA("va_readscript").setValue(null);
        getA("va_enroutetova").setValue(null);

        // Death of Veteran - FNOD Action
        getA("va_benefitsstopped").setValue(null);
        getA("va_deathrelatedinformationchecklists").setValue(null);
        getA("va_lookedupvetrecord").setValue(null);
        getA("va_processedfnodinshare").setValue(null);
        getA("va_processedfnodinshareexplanation").setValue(null);

        // 0820a - FNOD ACTIONS - I CERTIFY I SENT THE FOLLOWING
        getA("va_pmc").setValue(null);
        getA("va_nokletter").setValue(null);
        getA("va_21530").setValue(null);
        getA("va_21534").setValue(null);
        getA("va_401330").setValue(null);
        getA("va_other").setValue(null);

        // 0820a - DEATH OF NON-VET BENEFICIARY FOR STOP PAYMENT ACTION. I CERTIFY THAT I...
        getA("va_benefitsstopfirstofmonth").setValue(null);
        getA("va_willroutereportofdeath").setValue(null);
        getA("va_possibleburialinnationalcemetery").setValue(null);


        // Setting submit mode so that CRM doesn't complain
        getA("va_nameofreportingindividual").setSubmitMode('dirty');
        getA("va_readscript").setSubmitMode('dirty');
        getA("va_enroutetova").setSubmitMode('dirty');
        getA("va_benefitsstopped").setSubmitMode('dirty');
        getA("va_deathrelatedinformationchecklists").setSubmitMode('dirty');
        getA("va_lookedupvetrecord").setSubmitMode('dirty');
        getA("va_processedfnodinshare").setSubmitMode('dirty');
        getA("va_processedfnodinshareexplanation").setSubmitMode('dirty');
        getA("va_pmc").setSubmitMode('dirty');
        getA("va_nokletter").setSubmitMode('dirty');
        getA("va_21530").setSubmitMode('dirty');
        getA("va_21534").setSubmitMode('dirty');
        getA("va_401330").setSubmitMode('dirty');
        getA("va_other").setSubmitMode('dirty');
        getA("va_benefitsstopfirstofmonth").setSubmitMode('dirty');
        getA("va_willroutereportofdeath").setSubmitMode('dirty');
        getA("va_possibleburialinnationalcemetery").setSubmitMode('dirty');

        // Set required
        getA("va_nameofreportingindividual").setRequiredLevel("required");
        getA("va_readscript").setRequiredLevel("required");
        getA("va_enroutetova").setRequiredLevel("required");
        getA("va_benefitsstopped").setRequiredLevel("required");
        getA("va_deathrelatedinformationchecklists").setRequiredLevel("required");
        getA("va_lookedupvetrecord").setRequiredLevel("required");
        getA("va_processedfnodinshare").setRequiredLevel("required");
        getA("va_processedfnodinshareexplanation").setRequiredLevel("required");
        getA("va_pmc").setRequiredLevel("required");
        getA("va_nokletter").setRequiredLevel("required");
        getA("va_21530").setRequiredLevel("required");
        getA("va_21534").setRequiredLevel("required");
        getA("va_401330").setRequiredLevel("required");
        getA("va_other").setRequiredLevel("required");
        getA("va_benefitsstopfirstofmonth").setRequiredLevel("required");
        getA("va_willroutereportofdeath").setRequiredLevel("required");
        getA("va_possibleburialinnationalcemetery").setRequiredLevel("required");
    }
...
function CreateOutlookEmail2() {
    ...
    if (!roId) {
        roId = _originalRO;
        sendToRO = false;
        //VTRIGILI - 2014-11-12 Replace 2007 endpoint with 2011 OData call
        sojAddress = va_crm_udo_getSoj();
...
function RestrictPCRAccess(id, callback) {
    ...
    GetContactInformation(id, exCon, function (rtContact) {
...
function getUniqueRequestNumber() {
    ...
    ////VTRIGILI 2015-01-09 Add better error checking 
    //if (window.parent && window.parent.opener && parent.window.parent.opener.Xrm && parent.window.parent.opener.Xrm.Page.data && window.parent.opener.Xrm.Page.data.entity.getEntityName() == 'phonecall') {
    ...
    //if (phoneWindow && (phoneWindow.Xrm.Page.getAttribute('regardingobjectid').getValue() != null)) {
    ...
    //VTRIGILI 2015-01-09 Add better error checking 
    //VTRIGILI 2015-01-13 Add more entity types and restructure conditionals
    try {
        if ((window.parent != null) && (window.parent.opener != null) && (window.parent.opener.Xrm != null) && (window.parent.opener.Xrm.Page.data != null)) {
            phoneWindow = window.parent.opener;
            if ((phoneWindow.Xrm.Page.data.entity.getEntityName() == 'phonecall') && (phoneWindow.Xrm.Page.getAttribute('regardingobjectid').getValue() != null)) {
                vetId = phoneWindow.Xrm.Page.getAttribute('regardingobjectid').getValue()[0].id;
            } else if ((phoneWindow.Xrm.Page.getAttribute('va_regardingobjectid') != null) && phoneWindow.Xrm.Page.getAttribute('va_regardingobjectid').getValue() != null) {
                vetId = phoneWindow.Xrm.Page.getAttribute('va_regardingobjectid').getValue();
            }
        ...
        if (phoneWindow && (phoneWindow.Xrm.Page.getAttribute('va_ssn').getValue() != null)) {
            ssn = phoneWindow.Xrm.Page.getAttribute('va_ssn').getValue();
        }
        ...
        if (window.parent.opener && window.parent.opener.CrmRestKit2011 && vetId !== null && vetId !== undefined && ssn !== null && ssn !== undefined) {
            ...
            var query = Xrm.Page.context.getClientUrl() + '/XRMServices/2011/OrganizationData.svc' + "/" + "va_servicerequestSet"
        ...
        //VTRIGILI 2015-01-13 - TRY/Catch added to work around IE9 only bug in VA Code
        //      since plan is to go with IE11 upon release this work around allows
        //      us to ignore issue 
        requestNumber = 'Blank Forms ' + today;
        Xrm.Page.getAttribute('va_reqnumber').setValue(requestNumber);
    }
...
function setVeteranName() {
    ...
    if ((Xrm.Page.ui.getFormType() == CRM_FORM_TYPE_CREATE) && (Xrm.Page.getAttribute('va_originatingcallid') != null) && (Xrm.Page.getAttribute('va_originatingcallid').getValue() != null)) {
        ...
        //VTRIGILI - 2014-11-12 Replace 2007 endpoint with 2011 OData call
        //VTRIGILI - 2014-12-19 Add va_vetContactID functionality
        //var vetId = GetAttributeValueFromID('phonecall', origCallId, 'regardingobjectid', 'activityid');
        var vetId = null;
        if ((Xrm.Page.getAttribute('va_vetcontactid') == null) || (Xrm.Page.getAttribute('va_vetcontactid').getValue() == null)) {

            CrmRestKit2011.Retrieve('PhoneCall', origCallId, ['RegardingObjectId'], false)
            .done(function (data) {
                debugger;
                vetId = data.d.RegardingObjectId.Id;
            })
            .fail(function (err) {
                UTIL.restKitError(err, 'Failed to retrieve vet data');
                vetId = null;
            });
        } else {
            vetId = Xrm.Page.getAttribute('va_vetcontactid').getValue();
        }
        ...
        var vetName = null;
        if (vetId != null) {
            CrmRestKit2011.Retrieve('Contact', vetId, ['FullName'], false)
            .done(function (data) {
                debugger;
                vetName = data.d.FullName;
            })
            .fail(function (err) {
                UTIL.restKitError(err, 'Failed to get name data');
                vetName = null;
            });
        }
