 ...
 */
commonFunctionsVip.initalize = function (execCon) {
    ...
    _globalCon = Xrm.Utility.getGlobalContext();
    ...
    _KMRoot = _globalCon.getClientUrl().replace(_globalCon.organizationSettings.uniqueName, '') + 'isv/scripts_VIP/'; // used as a root for all scripts (claims, appeals, issue-based, etc.)
    ...
    _context = new vrmContext(_exCon);
};
...
//Depcrecated.  Use udo_CRMCommonJS instead
...
function InlineToolbar(containerId) {
    ...
    // VTRIGILI 2014-12-09 - No such container to hide in 2013
...
function SingleNodeExists(xmlObj, node_path) {
    //VTRIGILI - 2015-05-05 added check for null
