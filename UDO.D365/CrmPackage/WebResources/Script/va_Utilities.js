...
UTIL.isProperDate = function(fieldName) {
    ...
    year = parseInt(strYr);
    if (pos1 === -1 || pos2 === -1) {
        Xrm.Navigation.openAlertDialog({ text: "The date format should be: mm/dd/yyyy" });
        return false;
    }
    if (strMonth.length < 1 || month < 1 || month > 12) {
        Xrm.Navigation.openAlertDialog({ text: "Please enter a valid month" });
        return false;
    }
    if (strDay.length < 1 || day < 1 || day > 31 || (month === 2 && day > UTIL.daysInFebruary(year)) || day > daysInMonth[month]) {
        Xrm.Navigation.openAlertDialog({ text: "Please enter a valid day" });
        return false;
    }
    if (strYear.length !== 4 || year === 0) {
        Xrm.Navigation.openAlertDialog({ text: "Please enter a valid 4 digit year" });
        return false;
    }
    if (dtStr.indexOf(dtCh, pos2 + 1) !== -1 || UTIL.isInteger(UTIL.stripCharsInBag(dtStr, dtCh)) === false) {
        Xrm.Navigation.openAlertDialog({ text: "Please enter a valid date" });
        return false;
    }
    return true;
};
...
UTIL.openOutlookEmail = function (opts) {
    ...
    try {
        ...
        var msg = "Failed to create Outlook message: " + ex.description;
