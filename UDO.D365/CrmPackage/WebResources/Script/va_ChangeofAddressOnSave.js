...
function OnSave(context) {
    ...
    try {
        ...
        if (globalcadd.ca) {
            ...
            // Kirk 4/26/2018 Next check has been removed to allow address updating even if bank accounts are masked
    ...
    (va_mailingaddresszipcode != null && va_paymentzipcode != null && va_mailingaddresszipcode.toUpperCase().trim() != va_paymentzipcode.toUpperCase().trim())) {
        ...
        var updateBenefitClaimAddressCtx = new vrmContext(exCon);
        updateBenefitClaimAddressCtx.addressParameters = GetUserSettingsForWebservice(exCon);
        ...
        if (_hasAppeals) {
            var appellantAddressCtx = new vrmContext(exCon);
            appellantAddressCtx.user = GetUserSettingsForWebservice(exCon);
        ...
        if (_changingAwardAddress /*|| _hasAppeals*/) {
            ...
            updateBenefitClaimAddressCtx.user = GetUserSettingsForWebservice(exCon);
...
function CreateNote(serviceRequestId) {
    ...
    try {
        ...
        var createNoteCtx = new vrmContext(exCon);
        createNoteCtx.user = GetUserSettingsForWebservice(exCon);
