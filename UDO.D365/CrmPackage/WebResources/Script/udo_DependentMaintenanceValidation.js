...
var propertiesAppInsights = {
    "method": "Va.Udo.Crm.Scripts.DependentMaintenanceValidation", "description": "Called on Dependent Maintenance Validation"
};
...
function onLoad(execContext) {
    try {
        startTrackEvent("UDO Dependent Maintenance Validation", propertiesAppInsights);
    ...
    stopTrackEvent("UDO Dependent Maintenance Validation", propertiesAppInsights);
}
...
function ValidateSsn(execContext) {
    try {
        ...
        function validateSSNSeries(ssnString) {
            var re = /^(?!000|666)[0-8][0-9]{2}(?!00)[0-9]{2}(?!0000)[0-9]{4}$/;
            return re.test(ssnString);
        }      

        var crme_ssn_value = formContext.getAttribute("crme_ssn").getValue();

        if (crme_ssn_value === null || crme_ssn_value === "" || crme_ssn_value === "undefined") {
            ssnControl.addNotification({
                messages: ["Required fields must be filled in."],
                notificationLevel: "REQUIRED",
                uniqueId: ssnNotificationId
            });
        } else if (!validateNineDigitNumber(crme_ssn_value)) {
            ssnControl.addNotification({
                messages: ["SSN must be nine (9) digits"],
                notificationLevel: "ERROR",
                uniqueId: ssnNotificationId
            });
        } else if (!validateSSNSeries(crme_ssn_value)) {
            ssnControl.addNotification({
                messages: ["SSN is invalid"],
                notificationLevel: "ERROR",
                uniqueId: ssnNotificationId
            });
        } else {
