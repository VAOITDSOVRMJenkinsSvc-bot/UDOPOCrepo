...
var reports = {
    ...
    getReports: function () {
        ...
        doRequest(url);
        var columns = ["ReportId", "Name"];
        webApi.RetrieveMultiple("report", columns, null)
        .then(function(data){
            if (data && data.entities.length > 0) {
                len = data.entities.length;

                for (var i = 0; i < len; i++) {
                    me.list.push({ name: data.entities[i].Name, reportId: data.entities[i].ReportId });
                }
            }
        })
