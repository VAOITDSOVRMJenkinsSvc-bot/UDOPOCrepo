...
function HardcodeXmlResponse() { // skipMinSearchFields 
    ...
    //Corp Response
    xmlstring = '<return></return>'; HardcodeResponse(xmlstring, Xrm.Page.getAttribute("va_findcorprecordresponse"));
    ...
    //Dependent Response
    xmlstring = '<persons></persons>'; HardcodeResponse(xmlstring, Xrm.Page.getAttribute("va_finddependentsresponse"));

    //All Relations Response
    xmlstring = '<dependents></dependents>'; HardcodeResponse(xmlstring, Xrm.Page.getAttribute("va_findallrelationshipsresponse"));

    //General Information Response
    xmlstring = '<return><awardBenes></awardBenes></return>'; HardcodeResponse(xmlstring, Xrm.Page.getAttribute("va_generalinformationresponse"));

    //General Information By PID Response
    xmlstring = '<return><evrs></evrs><flashes></flashes><diaries></diaries></return>'; HardcodeResponse(xmlstring, Xrm.Page.getAttribute("va_generalinformationresponsebypid"));
    ...
    // Address
    xmlstring = '<return></return>'; HardcodeResponse(xmlstring, Xrm.Page.getAttribute("va_findaddressresponse"));

    // va_awardfiduciaryresponse
    xmlstring = '<return></return>'; HardcodeResponse(xmlstring, Xrm.Page.getAttribute('va_awardfiduciaryresponse'));
    //    }
...
function MarkAsRelatedVeteran(rtContact) {
    ...
    if (rtContact.isaveteran == '') { rtContact.isaveteran = false; }
    ...
    if (!_UserSettings) { _UserSettings = GetUserSettingsForWebservice(exCon); }
    ...
    if (isContact) {   //contact screen
        if (rtContact.firstName) { Xrm.Page.getAttribute('firstname').setValue(rtContact.firstName); }
        if (rtContact.lastName) { Xrm.Page.getAttribute('lastname').setValue(rtContact.lastName); }
        if (rtContact.middleName) { Xrm.Page.getAttribute('middlename').setValue(rtContact.middleName); }
        ...
        if (rtContact.firstName) { Xrm.Page.getAttribute('va_firstname').setValue(rtContact.firstName); }
        if (rtContact.lastName) { Xrm.Page.getAttribute('va_lastname').setValue(rtContact.lastName); }
        if (rtContact.middleName) { Xrm.Page.getAttribute('va_middleinitial').setValue(rtContact.middleName); }
        ...
        if (rtContact.gender && Xrm.Page.getAttribute('va_genderset')) {
            ...
            if (genderStr == 'M') { genderValue = _male; }
            else if (genderStr == 'F') { genderValue = _female; }
    ...
    if (Xrm.Page.data.entity.getEntityName() == 'phonecall') { CallerRelationToVeteranOnClick(true); }
    //debugger;
        ...
        if (!isContact) {
            if (idValue) {
                ...
                if (Xrm.Page.getAttribute("va_callerrelationtoveteran").getSelectedOption() != null && Xrm.Page.getAttribute("va_callerrelationtoveteran").getSelectedOption().text == 'Self') {
                    Xrm.Page.getAttribute("from").setValue([{ id: idValue, name: textValue, entityType: "contact" }]);
                }
                CheckPriorCalls();
            }
        }
...
function ShowFlagsAndTooltips(rtContact) {
    ...
    if (!_claimStatus) { _claimStatus = new ClaimStatus(); }
...
function SetHeaderField(fieldId, newText, textDecoration, underline, color, fontsize, cursor, tooltipText, isFooter) {
    if (typeof isFooter == "undefined") { isFooter = false; }
...
function LogMessages(error, warning, summaryOnly) {
    ...
    if (_VRMMESSAGE && _VRMMESSAGE.length > 0) {
        ...
        for (var i = 0; i < _VRMMESSAGE.length; i++) {
            // skip diag messages
            if (!_VRMMESSAGE[i].serviceName && !_VRMMESSAGE[i].methodName && (_VRMMESSAGE[i].description && _VRMMESSAGE[i].description.indexOf('did not execute correctly') != -1)) { continue; }
...
function CreateClaimServiceRequest(selection, srType, alreadyPrompted, defaultActionText, defaultActionId, callIssue, doOpen, doAlert) {
    //debugger;
    if (doOpen == undefined || doOpen == null) { doOpen = true; }
    if (doAlert == undefined || doAlert == null) { doAlert = false; }
    ...
    if (!alreadyPrompted) {
        if (!confirm('Please confirm that you want to create a new ' + srType + ' Service Request.')) { return; }
    }
    try {
        ...
        if (callIssue == undefined) { callIssue = null; }
        ...
        if (selection) {
            ...
            if (selection.claimSoj != undefined && selection.claimSoj) { claimSOJ = selection.claimSoj; }
        ...
        if (ptcpntId && ptcpntId != undefined && ptcpntId != '') { rtContact.participantId = ptcpntId; }
        ...
        if (defaultActionText && defaultActionText.length > 0 && (!defaultActionId || defaultActionId == undefined)) {
            switch (defaultActionText) {
                case '0820':
                    defaultActionId = 1; break;
                case '0820a':
                    defaultActionId = 953850001; break;
                case '0820d':
                    defaultActionId = 953850002; break;
                case '0820f':
                    defaultActionId = 953850003; break;
                case 'VAI':
                    defaultActionId = 2; break;
                case '0820 & VAI':
                    defaultActionId = 953850005; break;
                case 'Letter':
                    defaultActionId = 953850006; break;
                case 'Non Emergency Email':
                    defaultActionId = 953850007; break;
                case 'Other':
                    defaultActionId = 953850008; break;
                default:
        ...
        if (!personId) { personId = Xrm.Page.getAttribute("va_participantid").getValue(); searchCol = 'va_ParticipantID'; }
        ...
        if (Xrm.Page.getAttribute("va_generalinformationresponsebypid") || Xrm.Page.getAttribute("va_generalinformationresponse")) {
            ...
            if (awMain && awMain.length > 0) { genInfoXmlObject = _XML_UTIL.parseXmlObject(awMain); }
            if (awPID && awPID.length > 0) { genInfoByPIDXmlObject = _XML_UTIL.parseXmlObject(awPID); }
        ...
        if (VetMailingAddress.territory && VetMailingAddress.territory.length > 0) { terrProvince = VetMailingAddress.territory; }
        ...
        if (VetMailingAddress.milPO && VetMailingAddress.milPO.length > 0) { apoae = VetMailingAddress.milPO; }
        ...
        if (terrProvince.length > 0) {
            if (!VetMailingAddress.va_mailing_address1 || VetMailingAddress.va_mailing_address1.length == 0) { VetMailingAddress.va_mailing_address1 = terrProvince; }
            else if (!VetMailingAddress.va_mailing_address2 || VetMailingAddress.va_mailing_address2.length == 0) { VetMailingAddress.va_mailing_address2 = terrProvince; }
            else if (!VetMailingAddress.va_mailing_address3 || VetMailingAddress.va_mailing_address3.length == 0) { VetMailingAddress.va_mailing_address3 = terrProvince; }
            else {
   ...
   awardLineNode.selectSingleNode('//effectiveDate') && awardLineNode.selectSingleNode('//effectiveDate').text != '' ? FormatAwardDate(awardLineNode.selectSingleNode('//effectiveDate').text) : '';
        ...
        if (Xrm.Page.getAttribute("va_findmilitaryrecordbyptcpntidresponse").getValue()) {
            ...
            var branches = ''; var eod = ''; var rad = ''; var discharges = ''; var returnNodes = null; var latestTourDate;
                ...
                for (var i = 0; i < tourNodes.length; i++) {
                    if (tourNodes[i].nodeName == 'militaryPersonTours') {
                        ...
                        if (SingleNodeExists(tourNodes[i], 'eodDate')) {
                            ...
                            eod += tempdate + '\n';
                            if (latestTourDate == null) { latestTourDate = new Date(tempdate.split("/")[2], tempdate.split("/")[0] - 1, tempdate.split("/")[1]); }
                        }
        ...
        if (Xrm.Page.getAttribute("va_findpaymenthistoryresponse").getValue()) {
            ...
            if (paymentNodes.length == 0) { PaymentData.va_PaymentAmount = "00"; }
        }
        ...
        if (callerrelation != undefined && callerrelation) {
            ...
            if (callerrelation == '953850001') {
                callerrelation = '3';
            } else { callerrelation = '1'; }
        } else { callerrelation = '1'; }

        // verify dates
        try { if (vetDateOfDeath != null && new Date(vetDateOfDeath) < new Date('1/1/1900')) { vetDateOfDeath = null; } } catch (doe) { }
        // format
        if (vetDateOfDeath) { vetDateOfDeath = (new Date(vetDateOfDeath)).format("M/dd/yyyy"); }
        ...
        ServiceRequest.fail(function (error) {
           ...
           .done(function (data) {
               ...
               if (_currentEnv != null && _currentEnv != undefined && _currentEnv.srCode != null && _currentEnv.srCode != undefined) { scCode = _currentEnv.srCode; }
               if (_usingIFD) {
        ...
        CloseProgress(); alert("Error occurred.\n " + e.description);
        return null;
    }
    finally {
        CloseProgress();
...
function FormatPhone(areaCode, phoneNumber) {
    ...
    var sTmp = '';
    if (areaCode && areaCode != undefined && areaCode != '') { sTmp += areaCode; }
    if (phoneNumber && phoneNumber != undefined && phoneNumber != '') { sTmp += phoneNumber; }
    if (sTmp.length == 10) {
...
function ChangeOfAddressOnClick(selection) {
    ...
    if (!selection.ro && typeof _SetPrimaryTypeSubtype == 'function') { _SetPrimaryTypeSubtype('CAD', true, false); }
    ...
    if (Xrm.Page.data.entity.getEntityName() != 'contact') {            //phone screen
        if (Xrm.Page.getAttribute("regardingobjectid").getValue() &&
            ...
            var cadCode = '10001';
            if (_currentEnv != null && _currentEnv != undefined && _currentEnv.cadCode != null && _currentEnv.cadCode != undefined) { cadCode = _currentEnv.cadCode; }
            if (_usingIFD) { //
                /*
                Phone: 4210
                SR:    10012
...
function IsSingleAward() {
    ...
    if (generalInfoResponseXml) { generalInfoResponse_xmlObject = _XML_UTIL.parseXmlObject(generalInfoResponseXml); }
...
function executePostSearchOperations(searchForBIRLS) {
    ...
    if ((searchResults && searchResults.xml && searchResults.xml != '') || (vadirPersonXmlObject)) {
        ...
        var parsedCorpRecord = rtContactCorp.parseCorpRecord(searchResults, genXmlObject, poaXmlObject);
        if (searchForBIRLS) {
            var parsedBirlsRecord = rtContactBirls.parseBIRLSRecord(searchResults);
        }
...
function CreateDevNoteLogEntry(createNoteDetailresponseXml, devNoteText) {
    ...
    var noteId = null;
    if (createNote_xmlObject.selectSingleNode('//noteId') != null) { noteId = createNote_xmlObject.selectSingleNode('//noteId').text; }
    //if (noteId) {
...
CRMJSONStore.prototype.read = function () {
    if (!this.formAttributeName || this.formAttributeName == '') { throw 'Form attribute name must be set'; return; }
    if (!Xrm.Page.getAttribute(this.formAttributeName)) { throw 'Form attribute does not exist'; return; }
...
CRMJSONStore.prototype.overWrite = function (index, data, append) {
    ...
    if (append) {
        ...
        if (storedData[index] && storedData[index] != undefined) { storedData[index] = data; }
...
(function () {
    ...
    function exists(webServiceName) {
        ...
        for (var i in webServiceExecutionStatusList) {
            ...
            if (webService.name == webServiceName) { found = true; }
        }
    ...
    function getIndex(webServiceName) {
        ...
        for (var i in webServiceExecutionStatusList) {
            ...
            if (webService.name == webServiceName) { index = i; break; }
        }
    ...
    function add(webServiceConfig, append) {
        ...
        if (webServiceConfig instanceof Array) {
            ...
            if (!exists(webServiceConfig.name)) { crmJsonSource.write(webServiceConfig, append); }
            else { crmJsonSource.overWrite(getIndex(webServiceConfig.name), webServiceConfig, append); }
        }
    };
...
(function () {
    ...
    function exists(webServiceName) {
        ...
        for (var i in searchOptionsList) {
            ...
            if (search.name == webServiceName) { found = true; }
        }
...
(function () {
    ...
    function exists(searchName) {
        ...
        for (var i in searchList) {
            ...
            if (search.name == searchName && search.complete == true) { found = true; }
        }
