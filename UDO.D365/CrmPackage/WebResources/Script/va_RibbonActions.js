...
function PCR() {
    ...
    //Not used.
    var _sServerUrl = Xrm.Page.context.getClientUrl();
...
function Emergency() {
    ...
    var cols = ['va_WSLoginName', 'FullName', 'SiteId'];
    ...
    webApi.RetrieveRecord(userId, params.entity, cols, null)
			.then(function (data) {
                ...
                params.entity = "va_siteteams";
                return webApi.RetrieveMultiple(params.entity, ["va_Team"], filter)
			    //TODO: check for WebAPI error handler
			}).then(function (data1) {
				params.msg = 'No recipients to deliver the emergency notification were found for site "' + data.entity.SiteId.Name + '"';
                var columns1 = ['&$expand=teammembership_association($select=InternalEMailAddress'];
                
                //var expand1 = 'teammembership_association';
                params.entity = "Team";
				try{
                    var filter1 = 'TeamId eq guid\'' + data1.entities[0].va_Team.Id + '\'';
                }
                catch(err){
                    throw new Error(err);
                }
                ...
                return webApi.RetrieveMultiple(params.entity, columns1, filter1)
            }).then(function (data2) {
                ...
                switch(params.entity){
                    case "SystemUser":
                        params.msg = "The PCR does not have a site assigned!\r\n" +
                        "A Site must be assigned in order to get the list of recipients to deliver the emergency notification!"
                        break;
                    case "va_siteteams":
                        params.msg = "The PCR Site (NCC) does not have a Team assigned!\r\n" +
                    ...
                    case "Team":
                        ...
                        break;
                    default:
                        params.msg = "An unexpected error occured";
                        break;
                }			    
			    
			    //TODO: check for WebAPI error handler
			    Util.handleRespError(error, params.msg);
			    return;
			    });
}
...
FetchUtil.prototype._ExecuteRequest = function (sXml, sMessage, fInternalCallback, fUserCallback) {
    //TODO: convert SOAP to WebAPI request
...
FetchUtil.prototype._FetchCallback = function (xmlhttp, callback) {
    ...
    //TODO: this will need to adjust to fit the WebAPI data returned
