...
veteranGeneralInformation.prototype.performAction = function () {
    ...
    if (findGeneralInformationByFileNumberResponseXmlObject) {
        ...
        var successfulRequest = findGeneralInformationByPtcpntIdsService.executeRequest();

        if (successfulRequest && findGeneralInformationByPtcpntIdsService.responseXml
        ...
        if (!_searchCorpMin) {
            if (_searchCorp) {
                ...
                var succesfulRequest = findOtherAwardInformationService.executeRequest();

                if (succesfulRequest && findOtherAwardInformationService.responseXml
                ...
                var succesfulRequest = findIncomeExpensesByPtcptId.executeRequest();

                if (succesfulRequest && findIncomeExpensesByPtcptId.responseXml
            ...
            if (_searchVadir) {
                ...
                var succesfulRequest = fndAppeals.executeRequest();

                var appealKeyNode;
                var appealKeyTextNode;

                if (succesfulRequest && fndAppeals.responseXml && fndAppeals.responseXml != '') {
                ...
                if (appealKeyNode) {
                    ...
                    var succesfulRequest = getAppl.executeRequest();

                    if (succesfulRequest && getAppl.responseXml && getAppl.responseXml != '') {
