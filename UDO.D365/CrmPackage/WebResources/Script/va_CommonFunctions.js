...
var CRM_FORM_TYPE_COMPLETED_ACTIVITY = 4;
var WEBSERVICE_RESPONSE = '';

var _usingIFD = false;
var _KMRoot = null;
var _IFRAME_SOURCE_MULTIPLE = null;
var _IFRAME_SOURCE_SINGLE = null;
var _XML_UTIL = null;
var _context = null;

// Global variables
// For backward compatability ********************
var WebServiceURLRoot = null;
var _vrmVersion = null;
var _envName = null;
var _MVIServiceURLRoot = null;
var _MVIDAC = null;
var _PathwaysServiceURLRoot = null;
var _PathwaysDAC = null;
var _VacolsServiceURLRoot = null;
var _VacolsDAC = null;
var _globalDACUrl = null;
var _isPROD = null;

var _progressWindow = null;
var _noticeDlg = null;
var _allowToCashXMLResponses = true;    // if true will save xml responses in phone call, contact records. if false, will flush them on save
var _scriptWindowHandle = null;
var _logErrors = true;
var _logWarnings = false;
var _logQueries = true;
var _per = false;
var _suppressWSNotice = false;
var _progressWindowUrl = '/WebResources/va_progress.htm';
var _totalProgress = 0;
var _progressInterval = 1;
var _totalWebServiceExecutionTime = 0;
var _WebServiceResponse = null;
...
commonFunctions.initalize = function (execCon) {
    // Set the envrionment settings
    //window._currentEnv = window._currentEnv || environmentConfigurations.get(execCon);
    if (!window._currentEnv) {
        var envConfig = environmentConfigurations.initalize();
        envConfig.then(function (currEnv) { environmentSetup(currEnv) })
            .catch(function (error) { throw new Error(error.message); });
    }
    else {
    ...
    function environmentSetup(currEnv) {
        // Global variables
        // For backward compatability ********************

        WebServiceURLRoot = currEnv["CORP"];
        _vrmVersion = currEnv["Version"];
        _envName = currEnv["name"];
        _MVIServiceURLRoot = currEnv["MVI"];
        _MVIDAC = currEnv["MVIDAC"];
        _PathwaysServiceURLRoot = currEnv["Pathways"];
        _PathwaysDAC = currEnv["PathwaysDAC"];
        _VacolsServiceURLRoot = currEnv["Vacols"];
        _VacolsDAC = currEnv["VacolsDAC"];
        _globalDACUrl = currEnv["globalDAC"];
        _isPROD = currEnv["isPROD"];
        exCon = execCon;
        ...
        if (!WebServiceURLRoot || !currEnv.MVI || !currEnv.Pathways || !currEnv.Vacols) {
            UDO.Shared.openAlertDialog('Environment Configuration is not complete. One of the Web Service root URLs is not specified. Please contact System Administrator.');
        }

        //getClientUrl 
        _KMRoot = globCon.getClientUrl().replace(globCon.organizationSettings.uniqueName, '') + 'isv/scripts_VIP/'; // used as a root for all scripts (claims, appeals, issue-based, etc.)
        ...
        if (currEnv.isPROD || currEnv.name === 'PINT') { _usingIFD = true; }
...
function prepareProgress() {
    ...
    try {
        ...
        var serverURL = globCon.getClientUrl();
        _progressWindow = window.open(serverURL + _progressWindowUrl, 'SearchResults', params);
    }
    catch (err) { }
}
...
function ShowProgress(text) {
    ...
    //        _noticeDlg = showModelessDialog(serverURL + _progressWindowUrl, text, "status:false;scroll:off;dialogWidth:350px;dialogHeight:150px");
    //    }
...
//    }
//}
//depcrecated and removed contents.  Use udo_CRMCommonJS instead

function UserHasRole(roleName) {
    throw new Exception(error.message = "method not implemented.");
}
...
function SetDefaultCurrency() {
    throw new Exception(error.message = "This is used but not referenced");
}
...
function formatNumber(ctx) {

    try {

        var phoneNumber = ctx.getEventSource().getValue();
        ...
        if (phoneNumber !== null) {
            ...
            switch (sTmp.length) {
                case 10:
                    ...
                    ctx.getEventSource().setValue(formattedPhone);
                    //                oField.DataValue = â€œ(â€ + sTmp.substr(0, 3) + â€œ) â€ + sTmp.substr(3, 3) + â€œ-â€ + sTmp.substr(6, 4);
                    break;
                default:
                    //                UDO.Shared.openAlertDialog("Phone must contain 10 numbers.");
                    break;
            }
        }
    } catch (err) {
        UDO.Shared.openAlertDialog("Error in formatting phone number");
    }
...
function InlineToolbar(containerId) {
    ...
    if (!container) {
        return UDO.Shared.openAlertDialog("Toolbar Field: " + containerId + " is missing");
    }

    var btnTabIndex = container.tabIndex;

    // VTRIGILI 2014-12-30 - No such container to hide in 2013
    ...
    toolbar.AddButton = function (id, text, width, callback, imgSrc) {
        ...
        //VTRIGLI - 2015-01-21 Add check to preven duplicate button creation
        //          in CRM 2013, the page is reloaded after a create and with out
        //          this check button get duplicated
		...
		btStyle.Add("background-size", "100%");
        btStyle.Add("overflow", "visible");
        btStyle.Add("width", "98%");
		btStyle.Add("role","button");

        btn.style.cssText = btStyle.ToString();
        btn.addEventListener("click", callback);
		btn.addEventListener("keyup",function(e){if(e.keyCode==13 || e.keyCode==32){callback();}})
        btn.id = id;
...
var FormatExtjsDate = function (str_date) {
    if (str_date) {
...
parseXmlObject = function (xmlString) {
    if (xmlString === null) { UDO.Shared.openAlertDialog('XML Object contains a null value'); return null; }
...
SDK.MetaData = {
    ...
    _ServerUrl: function () {///<summary>
        /// Private function used to establish the path to the SOAP endpoint based on ctx
        /// provided by the formContext object or the ctx object returned by the GlobalContext object.
        ///</summary>
        var ServerUrl = this._Context().getClientUrl();
        if (ServerUrl.match(/\/$/)) {
            ServerUrl = ServerUrl.substring(0, ServerUrl.length - 1);
        }
        return ServerUrl + "/api/data/v9.1";
    },
    ...
    _associatedMenuConfiguration: function (node) {
        ...
        ///</summary>
        var orderValue;
        if (isNaN(parseInt(node.selectSingleNode("c:Order").text, 10))) { orderValue = null; }
        else { orderValue = parseInt(node.selectSingleNode("c:Order").text, 10); }
        return {
...
//var FullName = formContext.getAttribute('createdby').getValue()[0].name;
